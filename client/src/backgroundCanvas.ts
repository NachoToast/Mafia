...
function regenerateCanvas(): void {
    ...
    const squaresPerRow = Math.ceil((canvas.width - SPACING) / (SIZE + SPACING));
    const squaresPerCol = Math.ceil((canvas.height - SPACING) / (SIZE + SPACING));
...
// setInterval(() => {
//     currentY = (currentY + 0.1) % (SIZE + SPACING);
//     currentX = (currentX + 0.1) % (SIZE + SPACING);
