 ...
 * (a signed JWT).
 *
 * Site tokens should be provided in the `authorization` header
 * when making elevated requests.
 */
export interface SiteTokenPayload {
     ...
     *
     * This is used to make elevated requests to the Discord API
     * on behalf of this user.
     *
     * Since we only require the 'Identify' scope,
     * the only requests we can make are to:
     * - Terminate the current OAuth session (revoke).
     * - Extend the current OAuth session (refresh).
